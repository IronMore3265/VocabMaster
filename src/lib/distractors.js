 ...
 */
export function pickDistractors(target, pool, count, rng) {
  ...
  const picked = [];
  const seenDefinitions = new Set([normalizeDef(target.definition ?? '')]);
...
export function shortDefinition(definition, maxLength = 90) {
